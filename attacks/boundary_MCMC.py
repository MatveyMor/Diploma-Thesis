...
def boundary_attack_MCMC(net, initial_image, target_image, verbose=1, threshold=None, max_iter=None):
    if verbose > 0:
    ...
    while True:
        ...
        if np.argmax(prediction) == attack_class:
            ...
            distance_init = distance_last
            break
        else:
            epsilon *= 0.9
        prob_print = prob_init
    ...
    while True:
        ...
        while True:
            ...
            predictions = net(torch.tensor(trial_samples.reshape(15, 1, 28, 28)).to(dtype=torch.float)).data.numpy()
            n_calls += 10
        ...
        prob_step = np.max(softmax(net(torch.tensor(adversarial_sample.reshape(1, 1, 28, 28)).to(dtype=torch.float)).data.numpy()))
        argument = - (distance / distance_init) + prob_step / prob_init - 1
        alpha = np.exp(argument)
        ...
        if distance > distance_last:
            ...
            if alpha > 1: alpha = 1
            decision = np.random.choice([0,1], 1, p=[alpha, 1-alpha])[0]
            if decision == 0:
                adversarial_last = adversarial_sample
                distance_last = distance
                prob_print = prob_step
                eps_step = epsilon
                delta_step = delta
            else:
                adversarial_sample = adversarial_last
        ...
        if verbose > 0:
            sys.stdout.write("\rdistance: %0.4f, itetarion: %d, alpha: %0.3f, probability: %0.3f" % (distance_last, n_steps, alpha, prob_print))
        ...
        if not max_iter is None:
            if n_steps > max_iter:
                break
    print ('')
    return adversarial_last, prob_print